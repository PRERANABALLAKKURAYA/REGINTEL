...
class AIService:
    def __init__(self):
        ...
        settings_api_key = getattr(settings, "OPENAI_API_KEY", None)
        resolved_key = env_api_key or settings_api_key

        if resolved_key and not resolved_key.startswith("your_openai_api_key_here"):
    ...
    def generate_smart_answer(self, query: str, context: str, intent: str = "GENERAL_KNOWLEDGE") -> str:
        ...
        if intent in {"GENERAL_KNOWLEDGE", "EXPLANATION_REQUEST", "GUIDELINE_REQUEST", "REGULATION_REQUEST", "POLICY_REQUEST", "DOCUMENT_REQUEST"}:
...
If no documents are provided, you cannot answer this query - explain that no matching updates were found."""
        ...
        if self.api_key:
            try:
                ...
                # STEP 4: Add context if provided (documents or formatting instructions)
                if context.strip():
                    if "Authority:" in context or context.startswith("Document"):
                        user_message = f"Context documents:\n\n{context}\n\n---\n\nUser question: {query}"
                    else:
                        messages[0]["content"] += f"\n\nFormatting instructions: {context}"
                        user_message = query
                else:
                    user_message = query
                ...
                if "invalid_api_key" in error_text or "incorrect api key" in error_text or "401" in error_text:
                    print("[AI SERVICE] Invalid API key detected; switching to local response mode")
                    return self._generate_mock_answer(query, context, intent)
                if "insufficient_quota" in error_text or "429" in error_text:
                    print("[AI SERVICE] Quota/rate-limit issue detected; switching to local response mode")
                    return self._generate_mock_answer(query, context, intent)
                print(f"[AI SERVICE] Falling back to mock response")

        # STEP 7: Fallback when OpenAI is unavailable
        if not self.api_key:
            print("[AI SERVICE] No OpenAI API key configured; using local response mode")
            return self._generate_mock_answer(query, context, intent)

        return self._generate_mock_answer(query, context, intent)

    def _configuration_error_message(self, message: str) -> str:
        return (
            "⚠️ AI service configuration issue.\n\n"
            f"{message}\n\n"
            "After updating configuration, retry your question."
        )
